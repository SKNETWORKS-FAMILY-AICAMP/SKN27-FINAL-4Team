...
# + 명사 단정형(2026-07-16, v2 T01·T03 실측): "이름은 루비였잖아/지민이었지" — 지어낸
#   이름을 서술어 없이 단정하는 패턴이 기존 '했잖아' 계열을 빠져나감 (환각률 27% 사고).
#   과탐지("콩이잖아")는 2차 LLM 검증이 근거 대조로 거른다 — 시그니처 패턴 그대로.
_PAST_CLAIM = re.compile(
    r'했(었)?잖아|했었지|말했(었)?잖아|말했었|그랬잖아|한다고 했|다고 했(었)?잖아'
    r'|[이였]었?잖아|이었[지죠]|였[지죠]\b|였지\?'
    r'|[라다]고 했')   # '~(이)라고 했던 것 같은데' 헤지 보고형 (T01 '지민' 3연속 탈출구, 2026-07-19)
...
def _verify_llm(answer: str, evidence: str, question: str):
    ...
    resp = get_llm(temperature=0, max_tokens=40).invoke([
        ('system',
         ...
         "같은 것의 다른 이름도 근거 있음이다 ('로또'↔'복권', '사랑니 발치'↔'이빨 치료'). "
         "자구가 달라도 같은 사실이면 통과시켜라 — 과잉 차단이 누락보다 나쁘다.\n"
         "주의2-1: 고유명사(사람 이름·장소·직장명 등)를 제시하는 것은 '~였지?', '~맞아?', "
         "'~라고 했던 것 같은데'처럼 물음·헤지를 붙여도 단정이다. 그 고유명사가 컨텍스트에 "
         "없으면 근거 없음이다 (실측 사고: 기억에 없는 여동생 이름을 '지민이었지?'로 제시).\n"
...
def check_grounded(answer: str, evidence: str, question: str = '', crisis_turn: bool = False):
    ...
    # 빈 근거 결정 게이트 (2026-07-19, v2-off T01·T03 실측): 기억이 통째로 비었는데
    # 과거를 단정하면 검증 LLM에 물을 것도 없이 날조다 — 확률 검증자가 이 케이스를
    # 간헐적으로 놓쳐 환각률 20% 사고. 결정적 차단 (미스는 안전, 날조는 재앙).
    if not (evidence or '').strip():
        print('[answer_guard] 빈 근거 + 과거 단정 → 결정적 차단')
        return False, '기억에 없는 과거 단정'
