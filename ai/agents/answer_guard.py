...
구조 — 결정적 게이트 → LLM 확인 (위기 감지·만료 벡터와 같은 시그니처 패턴):
  ① 게이트: 답변에 과거 단정 신호어가 없으면 통과 (대부분의 턴 — 비용 0)
  ② 신호어가 있으면 검증 LLM 1회: 그 단정이 컨텍스트(기억+최근 대화)에 근거 있나
...
# 과거 대화 단정 신호어 — "네가 전에 ~했다/말했다"류 (게이트: 이게 없으면 검증 스킵)
_PAST_CLAIM = re.compile(
    r'했(었)?잖아|했었지|말했(었)?잖아|말했었|그랬잖아|한다고 했|다고 했(었)?잖아'
    r'|(자주|많이|계속|맨날|종종)\s*(얘기|말|이야기)')
...
def check_grounded(answer: str, evidence: str, question: str = ''):
    """(통과 여부, 근거 없는 단정 요지) 반환. 게이트 통과 시 LLM 검증 1회."""
    if not answer or not _PAST_CLAIM.search(answer):
        return True, None   # 과거 단정 자체가 없음 — 비용 0 통과
    try:
...
def retry_instruction(offending: str) -> str:
    """재생성 시 주입할 지시문."""
