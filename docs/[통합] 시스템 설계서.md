...
2. **대화 진행 및 턴 수 제어**: 
    ...
    - 1~3턴 초기 단계에는 분석 연산 없이 캐릭터 페르소나 지침에 기반한 기본 안부 및 다정한 공감 대화를 수행합니다.

    - 4턴 이상부터 KcELECTRA 감정 모델과 척도 간접 채점기가 백엔드에서 비동기 기동합니다. 4턴 째에 사용자의 대화가 일상 대화인지 분류하고, 일상 대화면 가벼운 챗칭으로 주제 전환을 유도하고, 고민 상담이 이어질 시 감정 분석 및 Neo4j RAG(심리이론)/LTM(장기기억)을 조회하여 캐릭터의 감정 표정(행동) 변화 및 차/BGM 음악 추천 카드를 동적 결합하여 응답합니다.
