---
tags: [SKN27기, 최종프로젝트, 챗봇, 흐름도, 통합, 확정본]
문서버전: v1.1 (확정 · 피드백 반영)
설명: 강사님 흐름도 + 멘토님 계층도/상세도 + v5.6 참고자료를 통합한 최종 확정 아키텍처
---
...
| MLOps / Feedback Agent (재학습 큐) | 2차 확장 과제 |
| 실시간 스트리밍 (문장 버퍼 단위 TTS) | 2차 확장 과제 — 이번엔 응답 단위 TTS |

### 확정 사항 요약
- 임베딩: **KcELECTRA** (잠정, KoBERT 비교 후 재검토)
- TTS: **ElevenLabs**
- 유휴 타이머: **10초**

### 참고: 사용자 식별
모든 저장/조회는 `user_id` 기준으로 이루어집니다 (로그인 세션 또는 디바이스 식별자). 흐름도에는 별도 노드로 표시하지 않았으나, DB 스키마 설계 시 모든 테이블에 `user_id` 컬럼이 필요합니다.
