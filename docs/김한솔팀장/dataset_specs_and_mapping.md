...
CREATE TABLE personas (
    ...
    persona_name VARCHAR(100) NOT NULL, -- e.g., '웰니스 메이트', '웰니스 메이트', '웰니스 메이트'
...
To map the user's cognitive patterns (Mindfulness triad: Situation -> Thought -> Emotion) and track psychological triggers over time, the long-term memory network in Neo4j is modeled with the following nodes and relationships:
...
* **연동 스펙**:
  ...
  - 캐릭터 에이전트 `웰니스 메이트`의 System Prompt에 다중 턴 공감 Few-shot으로 주입하여 친밀감 있는 위로 톤을 구현합니다.
...
### 2.3 AI허브 공감 및 위로 대화 (Persona Prompt Alignment)



* **사용 목적**: 3인 캐릭터 에이전트(웰니스 메이트)의 고유 말투(어투, 종결어미) 및 성격 일관성 확보.
...
* **연동 스펙**:
  ...
  - **웰니스 메이트 (위로)**: '~했구나', '~마음이었겠네' 류의 부드러운 말투 발화문 추출.



  - **웰니스 메이트 (직면)**: '~다', '~음', '팩트는 ~' 류의 건조하고 명확한 T형 말투 발화문 추출.



  - **웰니스 메이트 (행동)**: '~해봐요', '~어떨까요?' 류의 액션 및 코칭형 말투 발화문 추출.
...
* **사용 목적**: 캐릭터 위로 대화의 심리학적 근거 및 깊이 제공 (RAG 지식 베이스).



* **물리적 구조**: `AOS-project/02-도메인리서치/심리이론/` 내 138개 개별 Markdown 파일 (마음 이완, 수용전념, 공감 경청, 자존감, 대인관계 조종 및 방어 패턴 등).
...
* **대화 엔진 연동 아키텍처**:
  ...
  - 4턴 이상 진행된 상담 모드 대화에서 사용자의 최근 대화 맥락(Context)을 임베딩하여 유사도 상위 K개(예: K=3)의 심리 케어 기법 및 설명 청크를 추출합니다.



  - 추출된 심리 이론 문맥을 LLM System Prompt의 `{{psychology_theory_context}}` 영역에 동적으로 주입하여, 에이전트(웰니스 메이트)가 마음 건강 관점 원리에 입각한 신뢰도 높은 위로와 대안을 발화하도록 유도합니다.
