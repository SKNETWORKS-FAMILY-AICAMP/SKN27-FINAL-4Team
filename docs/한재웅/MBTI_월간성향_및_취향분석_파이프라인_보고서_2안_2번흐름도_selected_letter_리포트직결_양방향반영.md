...
## 제3의 흐름도: 3개 LLM 점수 산정 + 슈퍼바이저 통합

아래 흐름도는 기존 확정 MBTI 프로세스 흐름도를 대체하지 않는다. 기존 흐름도의 `D. 통과한 IE/SN/TF/JP 축 응답의 점수화` 단계만 내부적으로 확장한 흐름이다. 나머지 월간 그래프 표시 점수 계산, 선호 경향 결정, 월간 MBTI 조합, 근거 리포트 생성, 마이페이지 제공 흐름은 기존 확정 흐름도와 동일하게 유지한다.

핵심은 하나의 LLM이 답변별 점수를 바로 확정하지 않고, 동일한 `target_axis`, 질문, 답변을 세 개의 LLM 점수 산정기에 독립적으로 전달한 뒤 슈퍼바이저가 규칙 기반으로 최종 점수를 확정하는 것이다.

```text
1. LLM A, LLM B, LLM C가 같은 응답을 독립적으로 점수 산정한다.
2. 슈퍼바이저는 세 결과를 response_id별로 모은다.
3. 같은 점수가 2개 이상이면 최빈값을 최종 점수로 선택한다.
4. 세 점수가 모두 다르면 중앙값을 최종 점수로 선택한다.
5. 최종 score, coding_status, reason만 기존 월간 계산 흐름으로 넘긴다.
```
...
flowchart TD
    ...
    C -->|"예"| D0["통과한 IE/SN/TF/JP 축<br/>응답 점수 산정 시작"]
    C -->|"아니오"| G["해당 IE/SN/TF/JP 축은<br/>기준 선호 경향 적용"]

    D0 --> D1["LLM A 점수 산정<br/>score, coding_status, reason 산출"]
    D0 --> D2["LLM B 점수 산정<br/>score, coding_status, reason 산출"]
    D0 --> D3["LLM C 점수 산정<br/>score, coding_status, reason 산출"]

    D1 --> S["슈퍼바이저<br/>3개 점수 결과 수집"]
    D2 --> S
    D3 --> S

    S --> R{"통합 규칙 적용<br/>최빈값이 있는가?"}
    R -->|"예"| MV["최빈값을 최종 score로 선택"]
    R -->|"아니오"| MD["중앙값을 최종 score로 선택"]

    MV --> D["최종 응답 점수 확정<br/>score, coding_status, reason 구성"]
    MD --> D
...
이 흐름에서 슈퍼바이저는 새로운 MBTI 해석을 수행하지 않는다. 슈퍼바이저의 역할은 세 LLM이 산정한 점수 후보를 정해진 규칙으로 통합해 최종 `score`를 고르는 것이다. 따라서 점수 산정 단계의 변동성은 줄이면서도, 이후 월간 집계와 리포트 생성 구조는 기존 설계를 그대로 유지할 수 있다.
