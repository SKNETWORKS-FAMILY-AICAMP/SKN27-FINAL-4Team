...
- **감정 흐름 기반 강조 가중치 반영**:
  - 감정 흐름이 회복세를 보이는 '점수 상향' 흐름일 때에는 모든 키워드의 글자 크기와 여백을 동일하게 유지합니다. 대신 이완 키워드를 채움형 `primary`로 먼저 배치하고, 스트레스 키워드를 윤곽형 `secondary`(Weight 0.7)로 표시하여 가독성을 해치지 않으면서 시선의 우선순위를 조율합니다.
