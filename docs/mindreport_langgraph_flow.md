...
```mermaid
flowchart TD
    START(["리포트 생성 요청"]) --> SUP["MindReportSupervisorAgent<br/>State 생성 및 체인 관리"]
    SUP --> CRITERIA["생성 기준·그래프 검증 에이전트<br/>collect_and_check_criteria"]

    CRITERIA -->|"주간 5개 또는 월간 20개 충족"| EMOTION["마음 분석 에이전트<br/>score_and_analyze_emotion"]
    CRITERIA -->|"기준 미충족"| FALLBACK["폴백 리포트<br/>fallback_report"]

    EMOTION -->|"분석 성공"| CAUSE["원인 키워드 에이전트<br/>extract_and_classify_causes"]
    EMOTION -->|"분석 실패"| FALLBACK

    CAUSE -->|"원인 있음 또는 근거 있는 원인 없음"| NARRATIVE["분석 근거·실천 대안 에이전트<br/>generate_narrative_and_actions"]
    CAUSE -->|"에이전트 실행 실패"| FALLBACK

    NARRATIVE -->|"생성 성공"| VALIDATE["리포트 검증 에이전트<br/>validate_report"]
...
flowchart TD
    ...
    DATA -->|"기간·대화 집합·생성 기준 오류"| R1["생성 기준 에이전트 재실행"]
    ANALYSIS -->|"감정 패턴·점수 불일치"| R2["마음 분석 에이전트 재실행"]
    ANALYSIS -->|"키워드 근거 오류"| R3["원인 키워드 에이전트 재실행"]
    ANALYSIS -->|"점수·상태 노출, 직접 인용, 얕은 내용"| R4["서술 에이전트 재실행"]
