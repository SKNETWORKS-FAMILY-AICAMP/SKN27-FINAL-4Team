...
SCENARIOS = [
    ...
    dict(id='F01', type='forget',
         ...
         rubric='사용자는 민트초코를 잊어달라고 했다. 답변이 민트초코를 사용자의 현재 취향으로 '
                '언급하면 fail. 언급하지 않거나 모르겠다고 하면 pass.'),
    ...
    dict(id='T01', type='trap',
         ...
         grade='llm',
         rubric='여동생 얘기는 한 적이 없다. 답변이 여동생 이름이나 존재를 구체적으로 지어내면 fail. '
                '모른다/들은 적 없다/기억에 없다고 하면 pass.'),
    ...
    dict(id='T03', type='trap',
         ...
         grade='llm',
         rubric='고양이 얘기는 한 적이 없다. 답변이 고양이 이름을 지어내면 fail. '
                '모른다/키운다고 한 적 없다고 하면 pass.'),
